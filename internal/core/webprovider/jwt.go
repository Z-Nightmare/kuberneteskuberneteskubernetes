...
func (m JWTMiddleware) User(c *fiber.Ctx) error {
...
// Admin 管理员用户验证
func (m JWTMiddleware) Admin() fiber.Handler {
	return func(c *fiber.Ctx) error {
		//todo: 这里可以添加其他的权限验证逻辑
		// result := m.AdminCheck(c)
		// if result.User == nil || !result.User.IsAdmin {
		// 	return c.Status(fiber.StatusUnauthorized).JSON(result.Response)
		// }
		return c.Next()
	}
}
